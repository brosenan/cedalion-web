...
Logic.prototype.push = function(f) {
	this.stack.push(f);
	if(this.trailLevels.length < this.stack.length) {
		this.trailLevels.push(this.trailPtr);
	} else {
		this.trailLevels[this.stack.length - 1] = this.trailPtr;
	}
};
...
Logic.prototype.process = function(stackLevel) {
	...
	while(this.running && this.stack.length > stackLevel) {
		...
		this.undoTrail(this.trailLevels[this.stack.length]);
		func();
	}
	//if(this.running) {
		//if(this.trailLevels.length > this.stack.length) {
...
Logic.prototype.undoTrail = function(newPos) {
	while(this.trailPtr > newPos) {
		this.trailPtr--;
		this.trailVar[this.trailPtr].value = this.trailVal[this.trailPtr];
	}
}

Logic.prototype.pushTrail = function(variable, value) {
	if(this.trailPtr == this.trailVar.length) {
		this.trailVar.push(variable);
...
Logic.prototype.unify = function(term1, term2) {
	...
	// If a command, relate to the code
	if(typeof(term1) === "function" && term1.code) {
		term1 = term1.code;
	}
	if(typeof(term2) === "function" && term2.code) {
		term2 = term2.code;
	}
...
//if(typeof(val)==="undefined") throw Error("Undefined in bind");
	...
	//undo.variable = this;
	//logic.push(undo);
	logic.pushTrail(this, this.value);
	this.value = val;
};
...
logic.program.addBuiltin("parseTerm", 3, function(logic, term) {
	...
	if(value instanceof Variable) {
		...
		// If value is a command, parse the code
